...
REGISTERS = [
    ...
    ("306 / 307", "작업 속도 / 속도 비율", "W", "mm/s · %",
     "306 상한 100 mm/s(가속도 400 mm/s²). 307 은 태스크가 곱하지 않는다(비율은 컨트롤러가 적용)"),
    ...
    ("fill", NEW_FILL, "277", "스캔 구간 (TPAC 동기 신호)", "R", "코드",
     "0 없음 / 1 전진 스캔 / 2 줄 바꿈 상승 / 3 후진 스캔. 줄 시작·끝에서 바꾸고 sig_hold(0.25 s) 기다린 뒤 움직인다"),
...
# ========================================================= 3. 브리지 레지스터
BRIDGE = [
    ("fill", NEW_FILL, "Coil 16 / Reg 1 bit 0", "DO[0] 스캔 방향", "277 = 1 전진 -> 1, 3 후진 -> 0",
     "1 = Forward, 0 = Backward (0 -> 1 = 라인 리셋) · FC1 과 FC3 에 같은 값"),
    ("fill", NEW_FILL, "Coil 17 / Reg 1 bit 1", "DO[1] 스캔 플래그", "277 = 1·3 이고 290 = 6 -> 1",
     "1 = 유효 검사 구간(펄스), 0 = 동결 (줄 바꿈·줄 끝·일시정지)"),
    ("fill", NEW_FILL, "Coil 18 / Reg 1 bit 2", "DO[2] 호밍/초기화", "290 = 0·2·3·5 -> 1, 7·8 -> 0",
     "1 = 전체 카운터 리셋(격자 준비·격자 사이), 0 = 준비 완료(원점 도착)"),
...
def main() -> None:
    ...
    ws = sheet(
        ...
        "브리지는 좌표와 스캔 동기 신호를 내준다. 신호는 바뀔 때마다 50 ms 이상 유지하고, "
        "방향·리셋이 바뀌는 동안에는 스캔을 끈다. TPAC 과 모의기(mqtt_test/tpac_encoder_sim.py)가 "
        "동시에 붙어도 된다.",
