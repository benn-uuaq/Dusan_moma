...
**X 는 제로점 기준이라 0 과 -75.8 사이만 움직인다.** 실기가 내보내는 280~285 는
...
- **로봇 동작 속도 (2~100 %)**: 슬라이더로 값을 맞추고 **속도 전송**을 누르면 `doosan/robot/req/speed`(T-010)가 나간다. `10 / 25 / 50 / 75 / 100 %` 프리셋 버튼은 맞추는 즉시 보낸다. **작업 중에도 바로 반영된다** — 노드가 29999 `speed -set` 으로 전달한다.
