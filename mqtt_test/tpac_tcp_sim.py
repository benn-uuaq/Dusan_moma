    ...
    **물러나는 동작은 없다.** 실기에서 후퇴는 probe_c/l/r 과 goto_zero 에서만
    ...
    def _build_path(self) -> tuple:
        ...
        **X 는 제로점 기준이라 0 이하로만 움직인다.** 실제 로봇이 내보내는
        ...
        그 사이를 오갈 뿐이다. 로봇은 반시계로 훑고 제로점 좌표계를 **Rz 180
        도로 뒤집어** 내보내므로(dus_init.script pub_rel) X·Y 가 함께
        반전되어, 호 가운데 깊이가 + 로 나온다. 후퇴는 스캔 구간이 아니라
        probe/goto_zero 에서 일어나고 그때는 발행하지 않는다.
        """
        pts: list = []
        corners: list = []
