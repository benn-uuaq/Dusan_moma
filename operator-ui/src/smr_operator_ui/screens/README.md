...
- `main_screen.py`
  ...
  - `set_work_cell_label(text)`으로 현재 스캔 중인 격자 이름(예: `"A0 (1/9)"`)을 표시한다. 원통이 너무 커서 AMR이 원주를 구역(segment)으로 나눠 이동하고, 각 구역 안에서 Cobot이 세로 격자(grid)를 하나씩 스캔하기 때문이다. MQTT `job_cmd`의 `grid` 블록에서 온다(`docs/mqtt_topic_form.md` 참고).
- `all_screens.py`
