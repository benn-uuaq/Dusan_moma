...
- `all_screens.py`
  ...
  - `CobotJogScreen`은 관절·TCP 조그, 현재 위치 표시, 홈 위치와 시작 포즈 저장을 담당한다. 조그는 누르는 동안 움직이므로 `jog_pressed`와 `jog_released`를 모두 보낸다.
    - 레지스터 주소가 정해지지 않은 명령은 `set_enabled_commands()`로 버튼을 잠근다. 주소를 모른 채 쓰면 로봇이 예기치 않게 움직인다.
  - `IOStatusScreen`은 출력(OUT) 신호에만 ON/OFF 버튼을 둔다. 입력은 PLC가 정하는 값이라 조작 대상이 아니다.
