...
- `main_screen.py`
  ...
  - `apply_wall_position(가로_mm, 세로_mm)`으로 사각형 작업 모델의 현재 위치 점을 갱신한다. 값은 `robot/status/tcp_pose_zero`(레지스터 280~285, 원점 기준 상대좌표)에서 온다. 로봇 태스크가 베이스 좌표계로 `cur - zero`를 내므로, 가로 = `rel_y`, 세로 = `rel_z`(위 +)로 변환한다. (dusan_v3부터 원점이 Y+→Y-로 바뀌어 부호를 뒤집지 않는다 — `dus_probe_l.script` 참고.) 가로가 화면에서 어느 쪽으로 그려지는지는 `RectWorkView.to_px()`가 정하며, 지금은 원점이 오른쪽 끝에 오도록 뒤집어 그린다.
  - `set_work_cell_label(text)`으로 현재 스캔 중인 격자 이름(예: `"A0 (1/9)"`)을 표시한다. 원통이 너무 커서 AMR이 원주를 구역(segment)으로 나눠 이동하고, 각 구역 안에서 Cobot이 세로 격자를 하나씩 스캔하기 때문이다. MQTT `job_cmd`(전체 작업 시작)의 `job_info`/`scan` 값으로 격자 수를 계산해 구역 1·격자 0부터 표시하며(`docs/mqtt_topic_form.md` 참고), 그 뒤 구역/격자를 넘어가는 자동 진행은 Operator UI/로봇 쪽 책임이다.
  - 화면 오른쪽 끝에 `SpeedBar`(세로 슬라이더)를 두어 로봇 동작 속도 비율(2~100 %)을 조절한다. 작업 중에도 자주 만지는 값이라 메뉴 안이 아니라 바깥에 둔다. **끄는 동안에는 숫자만 따라오고 놓을 때 한 번 `speed_changed` 를 보낸다** — 매 픽셀마다 명령을 보내면 29999가 밀린다.
  - 로봇이 실제로 쓰고 있는 값은 `set_speed_scale()` 로 되돌려 받아 바와 `로봇 속도` 행에 함께 표시한다. 펜던트나 외부 MQTT로 바뀐 값도 여기로 들어온다. 이때는 슬라이더 시그널을 막아 되먹임을 끊는다.
- `all_screens.py`
  ...
  - `CobotJogScreen`은 관절·TCP 조그와 기준 위치 저장을 담당한다. 조그는 누르는 동안 움직이므로 `jog_pressed`와 `jog_released`를 모두 보낸다.
    ...
    - **조그 버튼은 누른 순간 `jog_pressed`를 딱 한 번만 보낸다.** held 동안 되풀이해 다시 보내면 로봇이 매번 새 speedj/speedl 스크립트를 실행하느라 STOPPED→RUNNING을 반복해 움직임이 덜컹거린다(2026-08-31 확인). 계속 움직이는 건 로봇 노드의 `jog_hold_time`(안전 타임아웃, 충분히 길게 잡음)이 맡고, 실제 정지는 뗄 때 나가는 `jog_released` → 29999 `stop` 하나로만 이뤄진다.
    - 조그는 로봇을 실제로 움직이므로 로봇 연결 여부로 `set_enabled_commands()`가 잠그거나 연다(`OperatorWindow._update_jog_enabled`). Modbus 레지스터가 아니라 30001 소켓을 쓰므로 레지스터 주소 유무와는 무관하다.
