...
- `all_screens.py`
  ...
  - `IOStatusScreen`은 표를 조회 전용으로 두고, 조작 버튼은 표 아래 `출력 제어` 영역으로 뺀다. 버튼을 표 칸 안에 넣으면 행 높이가 커져 화면이 답답해진다.
    - 출력(OUT) 신호에만 버튼을 만든다. 입력은 PLC가 정하는 값이라 조작 대상이 아니다.
    - 신호마다 테두리 카드로 묶어 어느 버튼이 어느 신호의 것인지 구분한다. `set_value()`는 표와 출력 제어의 현재 값을 함께 갱신한다.
